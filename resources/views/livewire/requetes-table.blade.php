        ...
        <tbody>
            @foreach($requetes as $requete)
                <tr class="border-b dark:00jborder-gray-700">
                    <td class="p-4">
                        <div class="flex items-center">
                            <input type="checkbox" value="{{ $requete['idRequete'] }}" wire:model="checkedRequtes"   class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:0focus:ring-primary-600" @if ($selectAll !== false) checked @endif > 
                        </div>
                    </td>
                    <td class="px-4 py-3">{{ $requete->requetesSql }}</td>
                    <td class="px-4 py-3">{{ $requete->user->name }}</td>
                    <td class="px-4 py-3">{{ $requete->version->dateVersion }}</td>
